...
$querySD = "SELECT SUM(invoice_header.net_value) AS `net_value`, employees.emp_id, employees.emp_name, t.target
			FROM invoice_header
...
while ($rowSD = $db->fetch_array($resultSD)) {
	$allNames[] = $rowSD['emp_name'];
	$allIds[]   = $rowSD['emp_id'];
	$target     = $rowSD['target'];
	$tmtd       = $target/$day;
	$netVal     = $rowSD['net_value'];
	//$amtd     = ($netVal/$tmtd)*100;
	$tDay       = $target/$numDays;
	$amtd       = ($netVal/$target)*100;
	$rRate      = $netVal/$day;
	$dataTable .= '<tr>';
	$dataTable .= '<td class="bold empName">'.$rowSD['emp_name'].'</td>';
