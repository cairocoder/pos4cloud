...
$queryRM = "SELECT SUM(invoice_header.net_value) AS `net_value`, employees.emp_id, employees.emp_name, t.target
			FROM invoice_header
			JOIN locations ON invoice_header.loc_id  = locations.loc_id
			JOIN   (SELECT locations.area_manager, SUM(target.desc) `target` FROM `target`
					JOIN locations ON target.loc_id  = locations.loc_id
					WHERE target.month = ".$month."
					AND   target.year  = ".$year."
					AND   locations.area_manager != 286
					GROUP BY locations.area_manager) t
			ON t.area_manager = locations.area_manager
			...
			AND   locations.area_manager != 286
			GROUP BY locations.area_manager";

$resultRM = $db->query($queryRM);
...
while ($rowRM = $db->fetch_array($resultRM)) {
	$allNames[] = $rowRM['emp_name'];
	$allIds[]   = $rowRM['emp_id'];
	$target     = $rowRM['target'];
	$tmtd       = $target/$day;
	$netVal     = $rowRM['net_value'];
	//$amtd     = ($netVal/$tmtd)*100;
	$tDay       = $target/$numDays;
	$amtd       = ($netVal/$target)*100;
	$rRate      = $netVal/$day;
	$dataTable .= '<tr>';
	$dataTable .= '<td class="bold empName">'.$rowRM['emp_name'].'</td>';
...
<br><hr><br>
<!--Div that will hold the column chart-->
<div id="tbl_div4" style="border:1px solid;margin:8px;padding:5px;">
	<table style="width:100%">
		<thead>
			<tr>
				<th>Area</th>
		...
		<tbody>
			...
			<tr>
				<td>KHOLOUD HASAN ALI</td>
				<td>112%</td>
				<td>12</td>
			</tr>
			<tr>
				<td>SAMEH AHMED MOHAMED</td>
				<td>112%</td>
				<td>13</td>
			</tr>
		</tbody>
...
<script type="text/javascript">
	...
	function drawChart() {
		var allNames    = Array();
		var totalTMonth = parseFloat($("#totalTMonth").text().replace(/,/g, ""));
		var totalAMTD   = Array();
		var TAMTD       = 0;
		...
		dataArray.push(["Remain", totalTMonth - TAMTD]);
		// Create the data table.
