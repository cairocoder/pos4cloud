<?php
    ...
    while ($rowGetSeason = $db->fetch_array($getSeason)) {
        $data .= '<tr>';
        $data .= '<td>'.$rowGetSeason['season_id'].'</td><td><span class="modify" uTbl="items_season" tblCol="season_id" uTblCol="desc" rowId="'.$rowGetSeason['season_id'].'">'.$rowGetSeason['desc'].'</span></td>';
        $data .= '</tr>';
    }
    $data .= '</table></div>';
    ...
    <div id='addNew' style='padding:10px; background:#fff;'>
        <table style="text-align:center">
            <tr>
                ...
                <td><input type="text" class="addNewVal" tblCol="desc" tbl="items_season"></td>
            </tr>
            <tr>
