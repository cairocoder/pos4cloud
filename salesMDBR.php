...
$queryBr = "SELECT SUM(invoice_header.net_value) AS `net_value`, locations.loc_id, locations.short_desc, t.target
			FROM invoice_header
...
while ($rowBr = $db->fetch_array($resultBr)) {
	$allNames[] = $rowBr['short_desc'];
	$allIds[]   = $rowBr['loc_id'];
	$target     = $rowBr['target'];
	$tmtd       = $target/$day;
	$netVal     = $rowBr['net_value'];
	//$amtd     = ($netVal/$tmtd)*100;
	$tDay       = $target/$numDays;
	$amtd       = ($netVal/$target)*100;
	$rRate      = $netVal/$day;
	$dataTable .= '<tr>';
	$dataTable .= '<td class="bold empName">'.$rowBr['short_desc'].'</td>';
...
<div id="tbl_div3" style="border:1px solid;margin:8px;padding:5px;">
	<?php
		$getEmployees = $db->query("SELECT employees.emp_id, employees.emp_name,
									SUM(invoice_header.net_value) AS totalValue,
									COUNT(invoice_header.net_value) AS totalQty
									FROM   employees
									JOIN   invoice_header ON invoice_header.sales_man_id = employees.emp_id
									WHERE  employees.loc_id = ".$locId."
									AND    MONTH(invoice_header.date) = ".$month."
									AND    YEAR(invoice_header.date)  = ".$year."
									GROUP BY employees.emp_id");

		$getEmployee = $db->query("SELECT SUM(IF(invoice_detail.type=1, invoice_detail.qty, -1*invoice_detail.qty))  AS `qty`
									FROM invoice_detail
									JOIN invoice_header ON invoice_header.invo_no = invoice_detail.invo_no
									AND  invoice_header.loc_id = invoice_detail.loc_id
									JOIN employees ON invoice_header.sales_man_id = employees.emp_id
									WHERE  employees.loc_id = ".$locId."
									AND MONTH(invoice_header.date) = ".$month."
									AND    YEAR(invoice_header.date)  = ".$year."
									GROUP BY employees.emp_id")
	...
	<table style="width:100%">
		<thead>
			<tr>
				...
				<th>Total Pieces/Invoice</th>
			</tr>
		</thead>
		<tbody>
			<?php if ($db->num_rows($getEmployees) > 0): ?>
				<?php
				while ($row  = $db->fetch_array($getEmployees)) {
					$array[] = $row;
				}
				while ($row2 = $db->fetch_array($getEmployee)) {
					$array[] = $row2;
				}
				...
				$emp_name   = array_values(array_filter($emp_name));
				$totalValue = array_values(array_filter($totalValue));
				$totalQty   = array_values(array_filter($totalQty));
				$qty        = array_values(array_filter($qty));
				?>
				<?php for ($i=0; $i < count($emp_id); $i++): ?>
					<tr>
						<td><?php echo $emp_id[$i] ?></td>
						<td><?php echo $emp_name[$i] ?></td>
						<td><?php echo $totalValue[$i] ?></td>
...
<script type="text/javascript">
	...
	function drawChart() {
		...
		for (var i = 0; i < allNames.length; i++) {
			...
			dataArray.push(["Remain", totalTMonth[i] - totalAMTD[i]]);
		}
	    // Create the data table.
...
<script type="text/javascript">
	...
	function drawChart2() {
	    ...
	    var data = new google.visualization.arrayToDataTable(dataArray);
	    // Set chart options
	    var options = {'title':'Bar Chart', 'width':470, 'height':500};
	    // Instantiate and draw our chart, passing in some options.
