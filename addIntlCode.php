<?php
    ...
    while ($rowGetIntlCode = $db->fetch_array($getIntlCode)) {
        $data .= '<tr>';
        $data .= '<td>'.$rowGetIntlCode['intl_code_id'].'</td><td><span class="modify" uTbl="items_intl_code" tblCol="intl_code_id" uTblCol="desc" rowId="'.$rowGetIntlCode['intl_code_id'].'">'.$rowGetIntlCode['desc'].'</span></td>';
        $data .= '</tr>';
    }
    $data .= '</table></div>';
...
<h1>Add International Code Window</h1>
<br><br>
    ...
    <div id='addNew' style='padding:10px; background:#fff;'>
        <table style="text-align:center">
            <tr>
                ...
                <td><input type="text" class="addNewVal" tblCol="desc" tbl="items_intl_code"></td>
            </tr>
            <tr>
